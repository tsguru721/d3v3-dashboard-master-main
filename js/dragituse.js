...
function colorBubble(d) { return d.region; }
...
function bubble(nations) {
  ...
  var dot = svg.append("g")
    ...
    .enter().append("circle")
      .attr("class", "dot")
      .style("fill", function(d) { return colorScale(colorBubble(d)); })
      .call(position)
  ...
  function init() {
    ...
    for(var yy = 1800; yy<2009; yy++) {
      ...
      interpolateData(yy).filter(function(d, i) { 
        dragit.data[i][yy-dragit.time.min] = [xScale(d.income), yScale(d.lifeExpectancy)];
